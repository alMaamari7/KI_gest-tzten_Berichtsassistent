...
class ValueChainAnlayser:
    ...
    def value_chain_mapping(self, stakeholder, nutzerdaten, muster):
        ...
        einvironment = self.nutzerdata_lesen(
            nutzerdaten,
            "einvironment"
        )
        time.sleep(40)

        governance = self.nutzerdata_lesen(
            nutzerdaten,
            "Governance"
        )
        time.sleep(40)

        polices = self.nutzerdata_lesen(
            nutzerdaten,
            "polices"
        )
        time.sleep(40)

        social = self.nutzerdata_lesen(
            nutzerdaten,
            "Social"
        )
        time.sleep(40)

        wertschoepfungskette = self.nutzerdata_lesen(
            nutzerdaten,
            "wertschöpfungskette.txt"
        )
        time.sleep(40)
        ...
        for i in range(3):
            ...
            with open(
                VALUE_CHAIN_PROMPTS_DIR / "Wertschöpfungskette-Mapping",
                "r",
                encoding="utf-8"
            ) as f:
                ...
                wertschoepfungskette_mapper_prompt = (
                    wertschoepfungskette_mapper_prompt.replace(
                        "<Muster>",
                        str(all_muster)
                        if all_muster
                ...
                wertschoepfungskette_mapper_prompt = (
                    wertschoepfungskette_mapper_prompt.replace(
                        ...
                        str(feedback)
                        if feedback
                        else "Keine FeedbackDaten"
                    )
                )

                #print(wertschoepfungskette_mapper_prompt)

                response_mapping_table = (
                ...
                bewertun = self.vollstaendigkeit(
                    prompt,
                    response_mapping_table
                )
                ...
                self.save_response_to_file(
                    str(bewertun),
                    VALUE_CHAIN_FEEDBACK_DIR / "mapping"
                )
        ...
        for i in range(2):
            ...
            with open(
                VALUE_CHAIN_PROMPTS_DIR / "identifikation von ESG",
                "r",
                encoding="utf-8"
            ) as f:
                ...
                esg_identifiter_prompt = esg_identifiter_prompt.replace(
                    "<Muster>",
                    str(all_muster)
                    if all_muster
                    else "Keine MusterDaten"
                ...
                esg_identifiter_prompt = esg_identifiter_prompt.replace(
                    ...
                    str(stakeholder)
                    if stakeholder
                    else "Keine Stakeholder"
                )

                esg_identifiter_prompt = esg_identifiter_prompt.replace(
                    "<Mapping-Tabelle>",
                    str(response_mapping_table)
                    if response_mapping_table
                    else "Keine Mapping"
                )
                ...
                response_esg_risk_chance_table = (
                    ai_helper.get_openai_response(
                        esg_identifiter,
                        esg_identifiter_prompt
                    )
                )
                ...
                self.save_response_to_file(
                    str(bewertung),
                    VALUE_CHAIN_FEEDBACK_DIR / "risk_chance"
                )
        ...
        table = (
            str(response_mapping_table)
            + str(response_esg_risk_chance_table)
        )

        spaltenname = (
            "Bereich,Prozess/Aktivität,Quelle der Information,Anmerkungen"
            "Bereich,Thema,ESG-Kategorie,Risikotyp,Chancen,Impact,"
            "Wahrscheinlichkeit,Gesamtbewertung,Priorität"
        )
    ...
    def feedback_lesen(self, path):
        stakeholder_path = VALUE_CHAIN_FEEDBACK_DIR / path
    ...
    def cvs_maker(
        ...
        def cvs_maker(
            ...
            for i in range(2):
                ...
                with open(
                    VALUE_CHAIN_PROMPTS_DIR / "csv_maker",
                    "r",
                    encoding="utf-8"
                ) as f:
