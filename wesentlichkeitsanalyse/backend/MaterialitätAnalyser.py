...
class MaterialitaetAnalyser:
    ...
    def materialtaetpruefung(self, stakeholder, value_chain, nutzerdaten, muster):
        ...
        for i in range(3):
          ...
          with open(
              MATERIALITY_PROMPTS_DIR / "Matrialitätsbewertung",
              "r",
              encoding="utf-8"
          ) as f:
            prompt = f.read()
            impact_prompt = prompt
            impact_prompt = impact_prompt.replace(
                "<Muster>",
                str(all_muster) if all_muster else "Keine MusterDaten"
            )
            impact_prompt = impact_prompt.replace(
                "<Stakeholder>",
                str(stakeholder) if stakeholder else "Keine StakeholderDaten"
            )
            impact_prompt = impact_prompt.replace(
                "<daten>",
                str(nutzerdaten) if nutzerdaten else "Keine NutzerDaten"
            )
            response_impact_prompt = ai_helper.get_openai_response(
                impact_analyser,
                impact_prompt
            )
            bewertung = self.vollstaendigkeit(
                prompt,
                response_impact_prompt
            )
            self.save_response_to_file(
                str(bewertung),
                MATERIALITY_FEEDBACK_DIR / "impact"
            )
            time.sleep(30)
        ...
        for i in range(3):
          ...
          with open(
              MATERIALITY_PROMPTS_DIR / "Finanziele_Impact",
              "r",
              encoding="utf-8"
          ) as f:
            prompt = f.read()
            financal_prompt = prompt
            financal_prompt = financal_prompt.replace(
                "<Muster>",
                str(all_muster) if all_muster else "Keine MusterDaten"
            )
            financal_prompt = financal_prompt.replace(
                "<ESG>",
                str(value_chain) if value_chain else "Keine ESGDaten"
            )
            financal_prompt = financal_prompt.replace(
                "<daten>",
                str(nutzerdaten) if nutzerdaten else "Keine NutzerDaten"
            )
            response_financal_prompt = ai_helper.get_openai_response(
                financal_analyser,
                financal_prompt
            )
            bewertung = self.vollstaendigkeit(
                prompt,
                response_financal_prompt
            )
            self.save_response_to_file(
                str(bewertung),
                MATERIALITY_FEEDBACK_DIR / "finance"
            )
            time.sleep(30)
        ...
        for i in range(3):
          ...
          with open(
              MATERIALITY_PROMPTS_DIR / "impact_und_finanziel_tabelle",
              "r",
              encoding="utf-8"
          ) as f:
            prompt = f.read()
            financal_impact_prompt = prompt
            financal_impact_prompt = financal_impact_prompt.replace(
                "<Finance>",
                str(response_financal_prompt) if all_muster else "Keine FinanceDaten"
            )
            financal_impact_prompt = financal_impact_prompt.replace(
                "<Impact>",
                str(response_impact_prompt) if value_chain else "Keine ImpactDaten"
            )
            response_financal_impact_prompt = ai_helper.get_openai_response(
                financal_impact_zusammenfuehrer,
                financal_impact_prompt
            )
            bewertung = self.vollstaendigkeit(
                prompt,
                response_financal_impact_prompt
            )
            self.save_response_to_file(
                str(bewertung),
                MATERIALITY_FEEDBACK_DIR / "impact_finance"
            )
            time.sleep(30)
        ...
        table = (
            str(response_impact_prompt)
            + str(response_financal_prompt)
            + str(response_financal_impact_prompt)
        )

        spaltenname = (
            "Thema,Schweregrad, Reichweite, Irreparabilität,Wahrscheinlichkeit,"
            "Gesamtscore,Impact-Wesentlichkeit,Priorität,Empfohlene Maßnahme"
            "Thema,Finanzielle Größenordnung,Wahrscheinlichkeit,"
            "Langfristige Auswirkungen,Gesamtscore Finanzielle Wesentlichkeit,"
            "Priorität, Empfohlene Maßnahme"
            "Thema,Impact-Wesentlichkeit,Finanzielle Wesentlichkeit,"
            "Gesamtbewertung,Priorität,LSME-Referenz,Finale empfohlene Maßnahme"
        )
    ...
    def cvs_maker(self, tabellen, tabellenanzahl, spaltenname):
        ...
        def cvs_maker(self, tabellen, tabellenanzahl, spaltenname):
            ...
            for i in range(2):
                with open(
                    MATERIALITY_PROMPTS_DIR / "csv_maker",
                    "r",
                    encoding="utf-8"
                ) as f:
                    csv_maker = f.read()
                    ...
                    respnse = self.save_response_to_file(
                        prufung,
                        FEEDBACK_DIR / "Stakeholder-Mapping" / "csv"
                    )
    ...
    def feedback_lesen(self, path):
        stakeholder_path = MATERIALITY_FEEDBACK_DIR / path
        ...
        if not nutzerdaten.strip():
            ...
            try:
                ...
                system_role_relevanz = (
                    ...
                    "zu analysieren und relevante Informationen direkt in "
                    "das unten stehende Relevanz-Muster einzutragen. Das Muster "
                    "enthält Hinweise, was in jede Kategorie gehört. Zusätzlich "
                    "hast du Zugriff auf Tabellen, die dir für jede Kategorie "
                    "und jedes Unterthema präzise Anforderungen an Vollständigkeit "
                    "und Detaillierungsgrad liefern."
                )
                ...
                with open(
                    ...
                    with open(
                        ...
                        # AI-Antwort abrufen
                        response = ai_helper.get_openai_response(
                            system_role_relevanz,
                            relevanz_prompt
                        )
    ...
    def delete_saved_responses(self, file_path: str):
        try:
            # Datei im Schreibmodus öffnen, ohne Inhalt
            with open(
                file_path,
                "w",
                encoding="utf-8"
